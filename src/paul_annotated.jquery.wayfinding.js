...
// I've made comments throughout this file to try to understand what is going on and what we need to change -- Paul
/* Based on what I've seen in this code, this is how we should integrate:
 * 		- Update build() and all the datastore generation.
 * 		- Confirm the datastore building is working and nothing else with the display breaks
 * 		- Update the routeTo() and all the pathfinding code
 * 		- Before changing the drawing stuff, make sure that a path is returned by our Module.pathfinding()
 * 		- Update the solution & drawing var building to get path animation working
 * 		- Confirm that all the switch statement options still work
 * 		- Run unit tests & linting & benchmarking
 */
...
(function ($) {
	...
	$.fn.wayfinding = function (action, options, callback) {
		...
		// Called by setStartPoint() & setEndPoint()
		// We shouldn't need to touch this
		...
		// This is only called by checkIds()
		...
		// This is only called by initialize switch statement
		// TL;DR > Error checking of maps. we won't change this
		...
		function checkIds(el) {
			...
			// This should give us an array of maps
			// We then hide the maps, add a new element that says some sort of load message
			status = $(el).find('div')
				.hide()
				.end()
				.append('<div id="WayfindingStatus" style="">' + options.loadMessage + '</div>');

			// If we don't have any maps, we have a problem....
			// maps gets set in getOptions()
			if (maps.length > 0) {
				
				// This is basically saying if you're looking at two differnt map divs but they have the same ID value, you need fix this
				...
				// The defaultMap var that gets set in getOptions() must be an existing map
...
//							portal.idB = innerSegment.id;
		...
		// Called by generateRoutes() & getShortestRoute()
		...
		// Only called by generateRoutes() & itself
		// We should NOT need this function anymore
		// Old dijkstra's madness here
		...
		// Only called by build()
		// We should NOT need this function anymore
		// This was for building all the routes from startpoint to all rooms I think
		...
		// Only called by getShortestRoute() & itself
		// Should be replaced by our pathfinding code
		...
		// Only called by routeTo()
		// Should be replaced by our pathfinding code
		// Going to ignore the details as they are confusing
		...
		// Only called by establishDataStore
		// We are going to replace this build code entirely
		...
		function build() {
			...
			// Shouldn't need route generation anymore...
			generateRoutes();

			// This is a pointless return statement
			// dataStore is a global variable since var was not used to define it here
			return dataStore;
		} // function build

		// Ensure a dataStore exists and is set, whether from a cache
		// or by building it.
		// Called primarilty by initialize(). The other times it is called is if startPoint or accessibleRoute are changed
		// But why does the datastore need to be established if that is the case. Should only happen once in our implementaion...
		// We will probably rip out the cache stuff. Should be a build the datastore once and be done.
		// No need for a cache because we made the build really fast :)
		
		function establishDataStore(onReadyCallback) {

			// Set as null initially unless the client passed in cache info
			// But where would that have come from?
			// With our implentation, there should be no need for a cache, so hopefully we can delete this all
			// Ignoring the details for that reason
			// I'm sure parsing the JSON string/object is tons of fun
			if (options.dataStoreCache) {
				...
				// This is where our build() implementation is called
				// dataStore might move locations
				dataStore = build();

				// Varies depending on what called establishDatastore
				// For initialize's call to it, a start point is set, the options are all set, and 
				// 	the loading screen is cleared, and it's callback is called
				if(typeof onReadyCallback === 'function') {
					onReadyCallback();
				}
			}
		...
		// Will be called by initialize if passed.maps is undefined
		// There's a lot of stuff here that I will come back to...
		// For now, I'm assuming (and hoping) that we don't need to change this
		// Called when the at the startpoint switch statement or in initialize()
		...
		function setStartPoint(point, el) {
			...
			// Weird function code we saw in getOptions()
		...
		// This gets called everytime wayfinding() is called for each object in this
		// Only called in the wayfinding() loop
		// Get any new options between calls or set up the options for the first time in case something was not provided
		// Set options based on either provided options or defaults
		// I don't think we will need to change this
		
		function getOptions(el) {
			
			// If we're initializing, the div has no data
			var optionsPrior = el.data('wayfinding:options');

			drawing = el.data('wayfinding:drawing'); // load a drawn path, if it exists

			// Extend is a confusing function but basically, it mashes the two objects together, in case the current options are missing anything
			...
			// check for settings attached to the current object
			// There's no clear reference to metadata from what i've seen. Can't find it on api.jquery.com
			...
			// I guess if defaultMap isn't set, the default settings make it a function that returns the value 'map.1'
			// Why not just set it to 'map.1' ???????
			...
			// Set startpoint correctly
			// Same weird syntax as defaultMap. Returns 'startpoint'
			if (typeof options.startpoint === 'function') {
				startpoint = options.startpoint();
			} else {
				startpoint = options.startpoint;
		...
		// Once the options are set up, and in case we've built the datastore and/or drawn a path
		// Save those things so we can check later
		// Called everytime wayfinding() is called for each object in this
		// Called in initialize() & in the wayfinding() loop
		// I don't think we will need to change this
		...
		// Some svg graphic hacks happen here that we can probably ignore
		...
		function cleanupSVG(el) { // should only be called once instead of twice if initalize and build for non datastore
			...
			// This happens with portals too...
			// If we have to change anything, it would be this stuff
		...
		// If we have to change anything, it will be the clicking of rooms functionality and how it does a recursive call to routeTo
		
		function activateSVG(el, svgDiv) {

			// Hide maps until explicitly displayed
			// Haven't we already hid them?
			$(svgDiv).hide();

			// Hide route information
			// Make the svgs clean looking
			...
			// Hopefully this won't be an issue with fixed svg files
			...
			// This lets you click somewhere and get routed to it
			// Something may change here
			...
			// This tells me that the maps arent originally added to the browser
			// So this is what makes the visible to the user
			$(el).append(svgDiv);
		...
		// Called when the currentFloor switch statement is hit
		// Makes a new map visible
		// Floors are switched when animating paths that go over multiple floors
		...
		// Only called by the animatePath switch statement
		// We won't need to change this
		...
		// If we build the drawing datastructure properly, this should remain unchanged
		// At least I hope that is the case because this code block is a lot to go over
		// Ignoring for now
		...
		// So this function will probably just call our pathfinding code...
		// We will need to change our pathfinding to return the right solution value, or at least the right info to set it
		// A lot of this function will need to change to work with our implementation
		...
		function routeTo(destination, el) {
			...
			if (startpoint !== destination) {
				...
				// This is where we will call Module.pathfinding() and build the solution from whatever we return
				solution = getShortestRoute();

				// This is drawing stuff which is important
				// reversePathStart will need to set to -1 if we encounter an error
				// Well, we should replace it with a better var name, like error 
				if (reversePathStart !== -1) {
					...
					// It looks like for each distinct floor traveled on, there is it's own set of drawing instructions
					drawing[0] = [];
					...
					// This is where we will encounter the most challenges integrating as our datastore implementation is completely different looking
					// The route is drawn using SVG path descriptions.
					// Really our solution should probably hold all this data.
					// x and y are the x1 and y1 values for path lines
					// m and n are the x2 and y2 values for the path lines. l is the length of the path	
					// This sets up the path for the start point
					// Depending on which end the path connects to the start door, determines the order that it is drawn
					// We will have to do a check here since we don't differ sides in our datastore implementation
					...
					// We need to fix the datastore accesses here as well
					// This does the same thing for each path/portal in the solution as we did for that starting path
					// Based off this code, doors shouldn't be considered valid entries in the solution
					// We will have to update our dijsktra to not visit neighboring doors
					// We will have to do a check here since we don't differ sides in our datastore implementation
					...
					for (i = 0; i < portalsEntered + 1; i++) {
						for (stepNum = lastStep; stepNum < solution.length; stepNum++) {
							...
							// There should only be one portal per drawing[i] entry
							// So after this, i increases by one
							// That means we need to grab both portal pieces here and set them up together?
					...
					// We should be able to ignore this part if we build everything correctly
...
//                                      drawing[level][i+1].type = "L";
		...
		// Removes that status div
		// Makes svgs visible I think
		// But only shows start floor based off default map
		// Probably only need to change the routeTo calls possibly
		// Only called by initialize()
		...
		// I'm sure we will need to change/adapt our code to fit this
		// Our Wayfinding constructor code is the equivalent of this
		
		function initialize(el, cb) {
			var mapsProcessed = 0;

			// We've made it past the checkIds() at this point
			// So we want to remove that status thing
			$('div:not("#WayfindingStatus")', el).remove();

			// Load SVGs off the network
			// This is nearly identitical to the code in the constructor
			// We'll have to add some of our vars here such as id_to_index
			$.each(maps, function (i, map) {

				var svgDiv = $('<div id="' + map.id + '"><\/div>');
				
				// Add id_to_index here probably
				...
				svgDiv.load(
					...
					function (svg, status) {
						if (status === 'error') {
							...
							// Is this required for us?
							// Our datastore building is smart enough to skip it
							// Well, we may need to clean these things up in order to display correct SVGs
						...
						// If all the maps load without error,
						if(mapsProcessed === maps.length) {
							// All SVGs have finished loading
							// Time to build the datastore
							// So we shouldn't change this but instead change the contents of establishDataStore
							establishDataStore(function() {
								// SVGs are loaded, dataStore is set, ready the DOM
								setStartPoint(startpoint, el);
								setOptions(el);
								replaceLoadScreen(el);
								// Callback is probably used here to make the client doesn't move forward until everything is done.
								// Since, I believe $.load is an async function
		...
		// If the action parameter is actually the settings, we are initializing and set options to the passed settings
		// Settings example:
		//{
		//	'maps': [
		...
		// this refers to the object retrieved by the jQuery selection.
		// Example: this = $('.all_maps')
		// In most cases I've seen, this is the div that holds all the svg map divs, so only one object
		...
		this.each(function () {
			...
			// Handle actions
			// So if wayfinding is called for the first time, with settings passed in, this is probably an initialize call
			// A switch statement to function call design pattern seems strange but apparently it's a thing...
			if (action && typeof (action) === 'string') {
				...
				case 'initialize':
					// passed is actually a reference to options
					// Actually not sure how references work in JS
					// Why not just use options?
					if (passed && passed.maps) {
						checkIds(obj);
						initialize(obj, callback);
					} else {
						// Shouldn't passed.showLocation be the same as options.showLocation?
						// Well, if for some reason they're not, set the startpoint
						// Why is this important? If passed.maps is undefined, then a start location doesn't matter, right?
				 ...
				 */
				// THIS SHOULD HAPPEN WHENEVER SOMEONE WANTS TO GET A ROUTE TO THE ROOM
				// Should happen once the datastore has been built
				// It's inside here that we will called our pathfinding code
				case 'routeTo':
					// call method
					routeTo(passed, obj);
					break;
				 ...
				 */
				// I believe this just redraws the path specified in drawing
				...
				// Shouldn't have to change anything really
				
				case 'startpoint':
					// change the startpoint or startpoint for the instruction path
					// If passed hasn't been set, simply return startpoint that is currently set
					if (passed === undefined) {
						result = startpoint;
					} else {
				...
				case 'currentMap':
					// return and set
					// If passed hasn't been set, simply return the current map id that is currently set
					if (passed === undefined) {
				...
				case 'accessibleRoute':
					// return and set
					// If passed hasn't been set, simply return if accessibleRoute is true or false that is currently set
					if (passed === undefined) {
				...
				// The title of this switch statement is misleading
				// This doesn't return or set the actual path route
				// This is to change the settings for how the path is drawn
				// Such as stroke color, stroke width, draw speed, and draw radius (not sure what radius is used for)
				// Shouldn't have to change anything really
				
				case 'path':
					// return and set
					if (passed === undefined) {
						// If passed hasn't been set, simply return the path settings that are currently set
						result = options.path;
					} else {
		...
		// If one of the switch statements was hit that could return a result (startpoint, currentMapId, etc), return it here
		// Shouldn't have to change anything really
		if (result !== undefined) {
			return result;
		}
